...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($conn, "SELECT farmer_id FROM farmers WHERE mobile = ? OR email = ?");
        if (!$stmt) {
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO farmers (name, mobile, email, address, district, state, farm_size, main_crops, password) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        if (!$stmt) {
            $errors[] = 'Database error: ' . mysqli_error($conn);
        } else {
            mysqli_stmt_bind_param($stmt, "ssssssdss", $name, $mobile, $email, $address, $district, $state, $farm_size, $main_crops, $hashed_pw);
            if (mysqli_stmt_execute($stmt)) {
                mysqli_stmt_close($stmt);
                header('Location: ' . BASE_URL . 'farmer/login.php?msg=registered');
                exit();
            } else {
                $errors[] = "Database error: " . mysqli_error($conn);
                mysqli_stmt_close($stmt);
...
<div class="auth-form-container">
    <div class="auth-form-card wide">
        <div class="auth-form-header">
            ...
            <p>Join our community to list crops and get insights.</p>
        </div>
        ...
        <form method="POST" action="" novalidate>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="district" value="<?php echo htmlspecialchars($old['district']??''); ?>">
                </div>
                <div class="form-group">
                    <label>State</label>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" name="farm_size" value="<?php echo htmlspecialchars($old['farm_size']??''); ?>">
                </div>
                <div class="form-group">
                    <label>Main Crops</label>
                    <input type="text" name="main_crops" placeholder="e.g. Rice, Wheat" value="<?php echo htmlspecialchars($old['main_crops']??''); ?>">
                </div>
            </div>
        ...
        <div class="auth-toggle">
            Already have an account? <a href="<?php echo BASE_URL; ?>farmer/login.php">Login</a>
        </div>
