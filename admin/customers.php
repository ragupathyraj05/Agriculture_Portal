...
if (isset($_GET['delete'])) {
    ...
    // Check for orders
    $check = mysqli_query($conn, "SELECT COUNT(*) as c FROM orders WHERE customer_id=$id");
    $count = mysqli_fetch_assoc($check)['c'];
    
    if ($count > 0) {
        $error = "Cannot delete customer with existing orders.";
    } else {
        // Fetch customer name for the log before deleting
        $cname_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT name FROM customers WHERE customer_id=$id"));
        $cname = $cname_row ? $cname_row['name'] : "ID #$id";

        if (mysqli_query($conn, "DELETE FROM customers WHERE customer_id=$id")) {
            $success = "Customer deleted successfully.";
            // Log activity
            $admin_id   = (int)($_SESSION['admin_id'] ?? 0);
            $action_type = 'User Deleted';
            $desc        = "Customer \"$cname\" (ID #$id) deleted by admin";
...
<div class="container mb-2">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
        <h1 style="color:var(--text-main);">Customers Configuration</h1>
    </div>
    ...
    <table class="admin-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="?delete=<?php echo $row['customer_id']; ?>" onclick="return confirm('Delete this customer? This cannot be undone.')" style="color:#ff4d4d;">
